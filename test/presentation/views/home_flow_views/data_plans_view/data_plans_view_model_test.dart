...
void main() async {
  ...
  setUp(() async {
    ...
    when(mockBundlesDataService.globalBundles)
        .thenReturn(getMockGlobalBundles());
    when(mockBundlesDataService.cruiseBundles)
        .thenReturn(getMockGlobalBundles());
  ...
  group("DataPlansViewModel Tests", () {
      ...
      if (vm.filteredCountries.isNotEmpty) {
        expect(
            vm.filteredCountries
                .any((c) => c.country?.contains("Afghanistan") ?? false),
            isTrue);
      }
    });
    ...
    test("login button navigation works", () async {
      ...
      verify(locator<NavigationService>().navigateTo(LoginView.routeName))
          .called(1);
    });
    ...
    test("notifications button navigation works", () async {
      ...
      verify(locator<NavigationService>()
              .navigateTo(NotificationsView.routeName))
          .called(1);
    });
    ...
    test("navigateToEsimDetail shows bottom sheet when user is logged in",
        () async {
      final DataPlansViewModel vm = DataPlansViewModel();
    ...
    test("navigateToEsimDetail navigates to login when user not logged in",
        () async {
      final DataPlansViewModel vm = DataPlansViewModel();
      final BundleResponseModel bundle = BundleResponseModel(
        bundleCode: "test123",
    ...
    test("isBundleServicesBusy returns correct loading state", () {
      ...
      when(locator<BundlesDataService>().isBundleServicesLoading)
          .thenReturn(true);
    ...
    test("processBanners with success response", () {
      ...
      final Resource<List<BannerResponseModel>?> resource =
          Resource<List<BannerResponseModel>?>.success(banners,
              message: "Success");
